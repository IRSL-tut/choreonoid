...
void exportPyValueTree(nb::module_& m)
{
    ...
    nb::class_<Mapping, ValueNode>(m, "Mapping")
        ...
        .def("find", [](Mapping* self, const std::string& key) -> ValueNode* { return self->find(key); })
        .def("findMapping", [](Mapping* self, const std::string& key) -> Mapping* { return self->findMapping(key); })
        .def("findListing", [](Mapping* self, const std::string& key) -> Listing* { return self->findListing(key); })
