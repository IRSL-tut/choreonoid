    ...
    : self(self)
{
    ...
    isTorqueOutputEnabled = org.isTorqueOutputEnabled;
...
void MuJoCoBody::getState()
{
    ...
    for(auto& info : linkInfos){
        ...
        if(info.hasActualJoint && info.qposadr >= 0){
            ...
            if(simImpl->isTorqueOutputEnabled){
                // Write back the drive effort generated by the actuator model.
                // This is not the net joint effort including gravity, contacts,
                // or external forces. The velocity feedforward term is included
                // because this plugin applies it as a drive force via
                // qfrc_applied rather than a native MuJoCo actuator.
                double u = 0.0;
                if(info.actuatorId >= 0){
                    u = d->qfrc_actuator[info.dofadr];
                    if(info.controlType == PositionControl && info.velocityFeedforward){
                        u += info.velocityGain * link->dq_target();
                    }
                }
                link->u() = u;
            }
...
void MuJoCoSimulatorItem::Impl::doPutProperties(PutPropertyFunction& putProperty)
{
    ...
    putProperty(_("Drive effort output"), isTorqueOutputEnabled, changeProperty(isTorqueOutputEnabled));
}
...
void MuJoCoSimulatorItem::Impl::store(Archive& archive)
{
    ...
    archive.write("drive_effort_output", isTorqueOutputEnabled);
}
...
void MuJoCoSimulatorItem::Impl::restore(const Archive& archive)
{
    ...
    archive.read("drive_effort_output", isTorqueOutputEnabled);
}
