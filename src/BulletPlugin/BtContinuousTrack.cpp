...
class SceneBtContinuousTrack : public SceneDevice
{
    BtContinuousTrack* trackDevice;
...
public:
    SceneBtContinuousTrack(BtContinuousTrack* device)
        : SceneDevice(device),
          trackDevice(device),
          shoesCreated(false),
          platesCreated(false),
...
bool readBtContinuousTrack(StdBodyLoader* loader, const Mapping* info)
{
    BtContinuousTrackPtr device = new BtContinuousTrack;
...
bool writeBtContinuousTrack(
    StdBodyWriter* writer, Mapping* info, const BtContinuousTrack* device)
{
...
SceneDevice* createSceneBtContinuousTrack(Device* device)
{
    return new SceneBtContinuousTrack(static_cast<BtContinuousTrack*>(device));
}
...
struct TypeRegistration
{
    TypeRegistration() {
        StdBodyLoader::registerNodeType(
            "BtContinuousTrack", readBtContinuousTrack);
        StdBodyWriter::registerDeviceWriter<BtContinuousTrack>(
            "BtContinuousTrack", writeBtContinuousTrack);
        SceneDevice::registerSceneDeviceFactory<BtContinuousTrack>(
            createSceneBtContinuousTrack);
    }
} registration;


BtContinuousTrack::Spec::Spec()
    : sprocketRadius(0.0),
      idlerRadius(0.0),
      numShoes(20),
      grouserHeight(0.01),
...
BtContinuousTrack::BtContinuousTrack()
    : spec_(new Spec)
{
}


BtContinuousTrack::BtContinuousTrack(const BtContinuousTrack& org, bool copyStateOnly)
...
const char* BtContinuousTrack::typeName() const
{
    return "BtContinuousTrack";
}


void BtContinuousTrack::copyStateFrom(const BtContinuousTrack& other)
{
    shoePositions_ = other.shoePositions_;
}


void BtContinuousTrack::copyStateFrom(const DeviceState& other)
{
    if(typeid(other) != typeid(BtContinuousTrack)){
        throw std::invalid_argument("Type mismatch in BtContinuousTrack::copyStateFrom");
    }
    copyStateFrom(static_cast<const BtContinuousTrack&>(other));
}


DeviceState* BtContinuousTrack::cloneState(DeviceState* /* existingClone */) const
{
    return new BtContinuousTrack(*this, true);
}


Referenced* BtContinuousTrack::doClone(CloneMap*) const
{
    return new BtContinuousTrack(*this);
}


void BtContinuousTrack::forEachActualType(std::function<bool(const std::type_info& type)> func)
{
    if(!func(typeid(BtContinuousTrack))){
        Device::forEachActualType(func);
    }
}


void BtContinuousTrack::clearState()
{
        ...
        if(!isPhaseAligned(spec_->numShoes)){
            ...
            if(adjusted != spec_->numShoes){
                ...
                MessageOut::master()->putWarningln(
                    formatR(_("BtContinuousTrack \"{0}\" (link \"{1}\"): "
...
int BtContinuousTrack::stateSize() const
{
...
const double* BtContinuousTrack::readState(const double* buf, int size)
{
...
double* BtContinuousTrack::writeState(double* out_buf) const
{
...
void BtContinuousTrack::addShoePosition(const SE3& pos)
{
    shoePositions_.push_back(pos);
}


void BtContinuousTrack::clearShoePositions()
{
    shoePositions_.clear();
}
